...
vector<User> GestionareCSV::loadUsers(const std::string &filename){
  ...
  string line;
  bool firstLine = true; // ignorăm prima linie
...
void GestionareCSV::saveUsers(const std::string &filename, const std::vector<User> &users){
  ...
  if(!file.is_open()){
    throw InvalidData("Nu se poate deschide fisierul pentru scriere: " + filename);
  }
...
vector<Cursa> GestionareCSV::loadCurse(const std::string &filename){
    ...
    while (getline(file, line)) {
      stringstream ss(line);
      string locPlecare, locSosire, dataPlecare, oraPlecare, locuriDisponibile, ID;
      ...
      curse.emplace_back(ID, locPlecare, locSosire, dataPlecare, oraPlecare, locuri);
    }
